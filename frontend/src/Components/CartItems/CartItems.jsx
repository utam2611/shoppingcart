...
const CartItems = () => {
  const { getTotalCartAmount, all_product, cartItems, addorder, removeFromCart } = useContext(ShopContext);
  { <hr /> }
    ...
    <Container key={id
      ...
      {all_product.map((e) => {
        if (cartItems[e.id] > 0) {
          ...
          return <>
            ...
            <hr />
            <Row>
              <Col><img src={e.image} className="carticon-product-icon" alt="" /></Col>
              <Col>{e.name}</Col>
              <Col>{e.new_price}</Col>
              <Col><button className="cartitems-quantity">{cartItems[e.id]}</button></Col>
              <Col>${e.new_price * cartItems[e.id]}</Col>
      ...
      <Row>
        <Col>
          ...
          <Row md="2" >
            <Col><Button md="" variant="secondary" size="lg" active onClick={() => {
              addorder((ida) => {
                for (let i = 0; i < ida.length; i++) {
                  console.log(ida[i])
                }
              })
            }}>Procced To Checkout</Button></Col>
          </Row>
        </Col>
        <Col>
